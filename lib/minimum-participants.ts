  ...
  const html = `
    <div style="${containerStyle}">
      ...
      <table style="${tableStyle}">
        <tr><th style="${thStyle}">Voucher Number</th><td style="${tdStyle}">${escapeHtml(voucherNumber)}</td></tr>
        <tr><th style="${thStyle}">Docket Number</th><td style="${tdStyle}">${escapeHtml(docketNumber)}</td></tr>
        <tr><th style="${thStyle}">Lead Passenger Name</th><td style="${tdStyle}">${escapeHtml(leadPassenger)}</td></tr>
        <tr><th style="${thStyle}">WhatsApp Number</th><td style="${tdStyle}">${escapeHtml(whatsapp)}</td></tr>
        <tr><th style="${thStyle}">Tour</th><td style="${tdStyle}">${escapeHtml(tourName)}</td></tr>
        <tr><th style="${thStyle}">Tour Date</th><td style="${tdStyle}">${escapeHtml(tourDate)}</td></tr>
        <tr><th style="${thStyle}">Participants</th><td style="${tdStyle}">${escapeHtml(participants)}</td></tr>
        <tr><th style="${thStyle}">Status</th><td style="${tdStyle}"><span style="color: #dc2626; font-weight: bold;">${escapeHtml(status)}</span></td></tr>
        <tr><th style="${thStyle}">Cancellation Date</th><td style="${tdStyle}">${escapeHtml(cancelledAt)}</td></tr>
        <tr><th style="${thStyle}">Agent Name</th><td style="${tdStyle}">${escapeHtml(agentName)}</td></tr>
        <tr><th style="${thStyle}">Agent Email</th><td style="${tdStyle}">${escapeHtml(agentEmail)}</td></tr>
        <tr><th style="${thStyle}">Internal Notes</th><td style="${tdStyle}">${escapeHtml(internalNotes)}</td></tr>
      </table>
...
async function getActiveParticipants(
  ...
  const { data, error } = await supabase.rpc('booking_participant_counts', { p_ids: [tourDateId] })
  if (error) throw new Error('Participant count could not be verified.')
  return Number(data?.[0]?.participants ?? 0)
}
...
async function getActiveReservationStatsMap(
  ...
  const { data, error } = await supabase.rpc('booking_participant_stats', {p_ids:tourDateIds})
  if (error) throw new Error('Participant statistics could not be verified.')

  for (const r of data || []) {
    // Build participants sum map
    activeParticipantsMap.set(
      r.tour_date_id,
      (activeParticipantsMap.get(r.tour_date_id) || 0) + Number(r.participants || 0)
    )
...
async function getActiveParticipantsMap(
  ...
  if (error) throw new Error('Participant statistics could not be verified.')
  for (const r of data || []) {
    map.set(r.tour_date_id, (map.get(r.tour_date_id) || 0) + Number(r.participants || 0))
  }
  return map
}
...
export async function resolveAlert(alertId: string, decision: 'KEEP_TOUR' | 'CANCEL_TOUR', notes?: string) {
  ...
  const { data, error } = await createClient().rpc('booking_resolve_alert', { p_actor: actor.id, p_id: alertId, p_decision: decision, p_note: notes ?? null })
  if (error || !data) return { success: false, error: error?.message ?? 'Decision was not saved.' }
  refreshBookingPages()
  after(async () => { await processEmailOutbox({limit:20}) })
  return { success: true, allProcessed: true, activeReservationsFound: data.reservationsCancelled,
    reservationsCancelled: data.reservationsCancelled, emailLogsCreated: data.emailLogsCreated,
    emailsSent: 0, emailsSkippedDuplicate: 0, emailsFailed: 0, details: [] as CancellationEmailDetail[],
    message: decision === 'CANCEL_TOUR' ? 'Tour and bookings cancelled together. Notifications queued.' : 'Keep decision saved.' }
}
...
export async function checkAndCreateAlerts(): Promise<CheckResult> {
  const { data, error } = await createClient().rpc('booking_check_minimum')
...
async function syncTourDateInternal(
  ...
  const { data, error } = await supabase.rpc('booking_sync_minimum', { p_id: tourDate.id })
  if (error || !data) throw new Error(error?.message ?? 'Minimum participants sync failed.')
  return data
}
