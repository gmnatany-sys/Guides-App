...
export default function EmailLogsClient({canSend}: {canSend:boolean}) {
  ...
  const loadData = async (append = false) => {
    ...
    try {
      const [logsResult, countResult] = await Promise.all([
        fetchEmailLogs(append ? emailLogs.length : 0),
        getPendingEmailCount()
      ])
      if (request !== requestId.current) return
      if (logsResult.error || countResult.error) throw new Error(logsResult.error || countResult.error || 'Unable to load email logs.')
      setEmailLogs(previous => append ? [...previous, ...logsResult.emailLogs as EmailLog[]] : logsResult.emailLogs as EmailLog[])
      setHasMore(logsResult.emailLogs.length === 50)
      setError(null)
  ...
  return (
    <div className="p-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {canSend && pendingCount > 0 && (
            <>
              ...
              >
                {sending ? 'Sending...' : 'Preview Pending (No Send)'}
              </Button>
              <Button
              ...
              >
                {sending ? 'Sending...' : `Send Next Batch (${pendingCount})`}
              </Button>
            </>
          )}
      ...
      {hasMore && <Button disabled={loading || sending} onClick={() => loadData(true)}>Load more emails</Button>}
    </div>
  )
}
